...
class generatorTable {
	...
	public function fastprint($action) {
		$this->fastprint = true;
		$this->action = $action;
	}
	
	public function print_button($name = 'submit',$value = 'Submit') {
		if ($this->rs->num_rows == 0) {$stat = 'disabled';}
		$tombol = '<input type="submit" name="'.$name.'" value="'.$value.'" '.$stat.'>';
		return $tombol;
	}
		...
		>
			...
			<tr align="center" height="25px" bgcolor="<?php echo $bgR; ?>" onmouseover="this.style.backgroundColor='#4E74DB';this.style.color='#ffffff'" onmouseout="this.style.backgroundColor='<?php echo $bgR; ?>';this.style.color='#000000'">
				...
				<?php
						//untuk penambahan cekbok 
						if ($this->fastprint) {
							echo '<input type="hidden" name="data[]" value="'.$row[0].'">';
						}
				?>
				...
				<?php if (is_array($this->delete)) {
					?><a href="<?php echo $this->delete['url']; ?>?id=<?php echo $this->idkolom.$this->addparam ?>&height=<?php echo $this->tinggiEdit; ?>&width=<?php echo $this->lebarEdit; ?>" class="thickbox"><IMG src="images/publish_x.png" alt="edit"></a><?php
				} else {
